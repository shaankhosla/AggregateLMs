"""Run a hyperparameter search on a RoBERTa model fine-tuned on MultiRC.

Example usage:
    python run_hyperparameter_search.py MultiRC/
"""
import argparse
import MultiRC
import data_utils
...
parser = argparse.ArgumentParser(
    description="Run a hyperparameter search for finetuning a RoBERTa model on the MultiRC dataset."
)
parser.add_argument(
    "data_dir",
    type=str,
    help="Directory containing the MultiRC dataset. Can be downloaded from SuperGLUE resources",
)
...
#    test_size=0.5,
#)
train_df = data_utils.process_jsonl(f"{args.data_dir}/train.jsonl", "</s>")
val_df, test_df = train_test_split(data_utils.process_jsonl(f"{args.data_dir}/val.jsonl", "</s>"), test_size=0.5,)
#test_df = data_utils.process_jsonl(f"{args.data_dir}/test.jsonl", "</s>")

tokenizer = RobertaTokenizerFast.from_pretrained("roberta-base")
train_data = MultiRC.MultiRCDataset(train_df, tokenizer)
val_data = MultiRC.MultiRCDataset(val_df, tokenizer)
test_data = MultiRC.MultiRCDataset(test_df, tokenizer)
